...
class Model:
    ...
    def predict(self, X):
        ...
        for i in range(len(self.layers)):
            ...
            # with dropout, use a random mask for hidden units
            if self.dropout and i < len(self.layers) - 1:
                cur_layer.m = np.random.randint(2, size=cur_layer.b.shape[0])
                input = input * cur_layer.m
        return input
    ...
    def train(self, X, y, alpha, beta, lmbda):
        ...
        # back propagation
        da = self.layers[-1].backward(f_x, y)
        for i in reversed(range(0, len(self.layers))):
            ...
            if i == 0:
                ...
                a_x = self.A_x[i - 1]
                if self.dropout:
                    dW = np.outer(self.layers[i - 1].forward(a_x) * self.layers[i - 1].m, da)
                else:
    ...
    def test(self, X):
        ...
        for i in range(len(self.layers)):
            ...
            # with dropout, hidden units' value * 0.5
            if self.dropout and i < len(self.layers) - 1:
                input *= 0.5
        return input
